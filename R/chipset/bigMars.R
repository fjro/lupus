...
y = genes$diseased
x = genes[,-c(1,34,96)]
fit1= earth(y=bigg, x=allBD,glm=list(family=binomial), degree=2)
summary(fit1)
...
require(pROC)
nrow(lupusBD)
testg = c(sample(1:nrow(safeControlBD), nrow(safeControlBD)*0.7), sample(30:nrow(safeLupusBD), nrow(safeLupusBD)*.7))
y = bigg
xtrain = safeBDAll[testg,]
ytrain = y[testg]

xtest = safeBDAll[-testg,]
ytest = y[-testg]
length(ytest)

preds=predict(earth(y = ytrain, x = xtrain, degree=1), newdata = xtest)
roc1=roc(ytest ~ preds)
dim(preds)
roc1$auc
plot(roc1)

fit = earth(y = ytrain, x = xtrain, degree=2, nfold = 2, ncross = 5)
preds=predict(earth(y = ytrain, x = xtrain, degree=2), newdata = xtest)
roc2=roc(ytest ~ preds)
roc2$auc
lines(roc2, col='red')

preds=predict(earth(y = ytrain, x = xtrain, degree=3), newdata = xtest)
roc3=roc(ytest ~ preds)
roc3$auc
lines(roc3, col='blue')

preds=predict(earth(y = ytrain, x = xtrain, degree=1, linpreds=T), newdata = xtest)
roc4=roc(ytest ~ preds)
roc4$auc
lines(roc4, col='green')

preds=predict(earth(y = ytrain, x = xtrain, degree=2, linpreds=T), newdata = xtest)
roc5=roc(ytest ~ preds)
roc5$auc
lines(roc5, col='yellow')

preds=predict(earth(y = ytrain, x = xtrain, degree=3, linpreds=T,), newdata = xtest)
...
marsProbes = data.frame(rep(0,length(marsRes)), rep(0,length(marsRes)), rep(0,length(marsRes)))
for (i in 1:length(marsRes))
{
  marsProbes[i,1] = marRes[i]
  marsProbes[i,2] = sum(marsProbes[i,1] %in% lllBig$X) + sum(marsProbes[i,1] %in% lllBig$Y)
  #marsProbes[i,4] = sum(marsProbes[i,1] %in% lll5$X) + sum(marsProbes[i,1] %in% lll5$Y)+ sum(marsProbes[i,1] %in% lll5$Z)
  marsProbes[i,3] = marsProbes[i,1] %in% resultsBig[1:119,8]
}

which(resultsBig$ProbeID == marsRes)

resultsBig[resultsBig$ProbeID == '35160_at',]
resultsBig[resultsBig$ProbeID == 'AFFX-DapX-3_at',]
resultsBig[resultsBig$ProbeID == 'AFFX-r2-Bs-dap-3_at',]
resultsBig[resultsBig$ProbeID == '226827_at',]
resultsBig[resultsBig$ProbeID == '213855_s_at',]
resultsBig[resultsBig$ProbeID == '1558756_at',]
resultsBig[resultsBig$ProbeID == '226842_at',]
resultsBig[resultsBig$ProbeID == '201432_at',]
resultsBig[resultsBig$ProbeID == '230737_s_at',]
